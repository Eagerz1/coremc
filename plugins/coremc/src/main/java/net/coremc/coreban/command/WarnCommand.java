...
/**
 * /warn <player> <tier> <reason>
 */
public final class WarnCommand implements org.bukkit.command.CommandExecutor {

    private final JavaPlugin plugin;
    private final PunishmentStorage storage;

    public WarnCommand(final JavaPlugin plugin, final PunishmentStorage storage) {
        this.plugin = plugin;
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (args.length < 3) {
            sender.sendMessage("/warn <player> <tier> <reason>");
            return true;
        }
        final int tier;
        try {
        ...
        if (sender instanceof final Player p && !CommandUtil.hasTierPerm(p, PunishmentType.WARN, tier)) {
        ...
        final Punishment p = CommandUtil.apply(plugin, PunishmentType.WARN, target.getUniqueId(), tier, reason,
                sender.getName(), now);
        if (target.isOnline() && target.getPlayer() != null) {
            CoreFoundation.getInstance().messages().sendRaw(target.getPlayer(),
                    CoreFoundation.getInstance().messages().getPrefix()
                    + " <yellow>You were warned (T" + tier + "): " + reason);
        }
